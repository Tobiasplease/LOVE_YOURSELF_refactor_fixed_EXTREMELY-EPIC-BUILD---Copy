...
void setup() {
  Serial.begin(9600);
  Serial.println("Hand Controller Ready - Pure Consciousness Mode");
  ...
  delay(1000);
  Serial.println("Ready for consciousness commands");
}
...
void updateServoPositions(unsigned long now) {
  ...
  for (int i = 0; i < NUM_SERVOS; i++) {
    if (now - lastUpdate[i] >= speeds[i]) {
      ...
      if (speeds[i] <= 3) {
        stepSize = 8;  // INSTANT: 8 degrees per step for startle reactions
      } else if (speeds[i] <= 10) {
