...
class CameraReactivityEngine:
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, float]:
        ...
        self.activity_level = self.smoothing_factor * self.activity_level + (1 - self.smoothing_factor) * boosted_activity
    ...
    def _check_pause_conditions(self, current_time: float) -> bool:
        ...
        if activity_spike and current_time - self.last_pause_time >= self.pause_cooldown:
...
def test_camera_reactivity():
    ...
    try:
        while True:
            ...
            cv2.putText(frame, f"Activity: {metrics['activity_level']:.3f}", (220, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

            # Chaos multiplier
            cv2.putText(frame, f"Chaos: {metrics['chaos_multiplier']:.2f}x", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)

            # Speed multiplier
            cv2.putText(frame, f"Speed: {metrics['speed_multiplier']:.2f}x", (150, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)

            # Sudden change indicator
            if metrics["sudden_change"] > 0.1:
                cv2.circle(frame, (width - 30, 30), 20, (0, 0, 255), -1)
                cv2.putText(frame, "!", (width - 35, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
