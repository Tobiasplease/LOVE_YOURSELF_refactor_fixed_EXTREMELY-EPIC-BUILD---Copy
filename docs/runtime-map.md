...
| [interior] felt arc — TURN: `Earlier you felt "{old}". More recently: "{new}".` / STEADY: `You've felt "{felt}", or near it, for {duration}.` | **THE EMOTIONAL ARC CHANNEL (Sep 4)** — `prompts.get_felt_arc_line`, registry `caption.felt-arc-turn`/`-steady`. Every mood read joins `context_compressor.felt_history` (cap FELT_HISTORY_MAX); the line states the trajectory as FACT in the machine's OWN felt phrases — turn fires once when the valence-class streak flips after a ≥20-min prior hold, steady after FELT_ARC_AFTER_S (1h) of one tenor; dosed FELT_ARC_MIN_GAP_S (30 min), displaced when live. The reflection's yourself/time organs also receive the day's sampled trajectory (`felt_arc` diet, `_add_felt_arc`) — the identity engine finally distills from days it actually felt. Fact in, meaning out; no scripted affect. Test: debug/test_felt_arc.py | NEW — watch for the line in quiet prompts and whether TRAITs start carrying felt history |
...
| [interior] "Preoccupied with: ..." (+ arc tail: "You've wanted this {duration}, and been refused N times.") | desire, gated to 3 injections + redose. **B3 (Aug 31): the want has a LIFECYCLE now** — `utils/want_ledger.py` records formed/affirmed/refused/acted/became for every want (event_log/want_ledger.json, seeded with the standing want's true age). The distill template lost its "or want to draw" nudge (wants are anything the reflection finds) and gained the BECAME slot: when a want changes, the machine names what the old one turned into, in its own words — that's the outcome stored on the closed entry. Refusals only count when the want was drawing-shaped (hunger-fired refusals hit the clock, not the want). The arc tail appears once the want is old (WANT_ARC_TAIL_AFTER_S, 6h) or refused; the reflection prompt receives the standing want's age/refusal facts past WANT_REFLECTION_FACT_AFTER_S (1d) or 2 refusals, plus the last 2 ended wants with their outcomes. Facts in, meaning out — curdling is the machine's move | NEW — watch `[🪞] Distilled` lines for BECAME, and want_ledger.json for outcomes |
...
| after each reflection | **DISTILLATION (the identity engine, June 28)**: `context_compression.distill_reflection` pulls TRAIT/BELIEF/WANT from the long-form reflection (plain, temp 0.3, _valid_self_fact gate, _roughly_same desire persistence) — **+ NAME/LORE harvest slots (Sep 3 evening, re-entry round)**: if the reflection named the machine or kept an imagining, the distiller collects it ("or none" — harvest, never invitation); NAME → ledger name slot + identity dose + awakening, UNDERSTANDING → a thread (match-or-extend; slot reworded from 'LORE — a story' same evening — deepening understanding, not story-production), QUESTION (Sep 4, attention round) → open-questions store in the ledger (cap 8, overlap-dedupe), re-entering as the 5th memory-surface source (`caption.question`) | core_facts.self (persona), current_belief, current_desire, self_name, threads — the Reflect→Become loop, now with a channel for the machine's own developing readings of things. Replaced the inert compression-thread introspection/self-synthesis |
...
**INVESTIGATE GLANCES (Sep 4, attention round — docs/attention-round-sep4.md):**
`pick_glance_target` gains a third kind: with INVESTIGATE_WEIGHT (0.25) the
gaze commits to a FAMILIAR STRANGER — conf ≤ INVESTIGATE_CONF_CEILING (0.35),
hits ≥ INVESTIGATE_MIN_HITS (500); the live map carried a wall lamp at 783k
sightings, conf 0.20 — uncertainty-weighted, 15-min per-term cooldown. The
cycle carries the attested fact (`caption.investigate`: "seen it many times
without ever being sure of it"), the close look accepts the glance, and
discernment verifies it. Glance choices + checks are logged facts now
(`glance_start`/`glance_check`). "What IS that?" stays the machine's move.
