...
| [💭] "A daydream, while nothing moved: …" | **THE STORY BEAT (Sep 2, stream variant — artist's call)**: on deep stillness (`unchanged_duration_s` ≥ STORY_BEAT_AFTER_S 2700s, at most once per STORY_BEAT_MIN_GAP_S 3600s, never on hot salience) a cycle becomes a story turn — no image, the one deliberately HOT slot (STORY_BEAT_TEMP 0.95; the cold distiller stays cold), seeded with REAL material (last episodic events, the standing want + refusals, the last two ended wants with their BECAME outcomes). Output enters the STREAM framed as reverie (`story.stream-frame` — conflation law: marked at the seam, can never read as scene truth) so the mind can carry its daydream forward. **FIREWALL**: story output never reaches observe()/add_caption/hour_log/recent_captions — fiction can never become a familiar concept, a compressed fact, or reflection material. Registry story.system/story.ask/story.stream-frame, panel-editable. Watch [💭] in logs (action=story_beat); if story-residue appears in later captions as scene truth, the retreat is the side-store variant | NEW — the "invent stories / emotional weather" experiment; first firings expected in dead-quiet stretches |
