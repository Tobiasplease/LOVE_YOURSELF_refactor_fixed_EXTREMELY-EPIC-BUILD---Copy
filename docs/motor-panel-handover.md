...
| lefthand | `/dev/arduino_lefthand` @9600 | `HAND,f0,f1,f2,f3` (composite), `SERVO,4/5/6,{d}` — `MOOD,{e}` + `LEFT_ARM_ENABLE/DISABLE` are wanderer-era, IGNORED by clean firmware (panel buttons removed July 27; the temperament system replaced them) | fingers 0-180/90; elbow (pin 4), shoulder (pin 5), wrist (pin 6) from config `LEFT_ARM_*_LIMITS` (60-120, creeping). Device sliders LIVE-track playback/generation since July 27 |
