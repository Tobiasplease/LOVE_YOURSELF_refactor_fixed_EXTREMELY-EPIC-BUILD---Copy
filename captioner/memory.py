...
class MemoryMixin:
    ...
    def absorb_detection(self, labels: list[str], timestamp: float | None = None):
        ...
        for label in labels:
            entry = self.motif_presence.setdefault(label, {"count": 0, "confidence": 0.0, "last_seen": 0})
            entry["count"] += 1  # type: ignore
            entry["confidence"] = min(entry["confidence"] + 0.1, 1.0)  # type: ignore
    ...
    def format_detected_labels(self) -> str:
        ...
        out = []
        for label in labels:
            out.append("I think that's a person" if "person" in label.lower() else f"That might be a {label}")
        return " ".join(out)
