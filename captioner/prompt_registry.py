...
FRAGMENTS = {
    ...
    "monologue.self-wrap": {
        "title": "Self-knowledge wrapper — RETIRED Sep 8",
        "text": "",
        "note": "RETIRED Sep 8 (artist: 'it bakes a hard instruction for what the captions should look like instead of encouraging the model to reason about what it is'). Proof the same morning: the standing line 'I measure distance to a task repeatedly instead of starting it' produced captions that MEASURED DISTANCES ('1m behind me:', '2m away:') until the stream was a list of stubs. A description of itself standing in the frame is stage direction, however honest — the North Star's own anti-pattern (model-generated affect re-injected verbatim). Identity now enters as material for reasoning: monologue.self-thought (a dated conclusion from its own 'yourself' reflection) and monologue.self-question (a question it is carrying). Kept as an empty fragment so overrides.json entries do not dangle.",
        "used_by": [],
        "placeholders": ["self_knowledge"],
    },
    "monologue.self-thought": {
        "title": "Identity — what you got to, thinking about yourself",
        "text": ' {when}, thinking about yourself, you got to: "{thought}"',
        "note": "Sep 8. The last sentence of the machine's most recent reflection on ITSELF, with its age — a past thought it can build on, revise or reject, not a standing fact about what it is. Rides on the identity dose (IDENTITY_EVERY_N_CAPTIONS), never every caption. Structure only: the thought is its own words.",
        "used_by": ["caption", "caption_blind"],
        "placeholders": ["when", "thought"],
    },
    "monologue.self-question": {
        "title": "Identity — a question you are carrying",
        "text": ' A question you\'ve been carrying: "{question}"',
        "note": "Sep 8. The least-recently surfaced of the machine's OWN open questions (lore ledger, distilled from its reflections). An open door, not a description. We never write the questions — 'who am I', 'where am I' are the machine's to ask, in the yourself-reflection where it is invited to.",
        "used_by": ["caption", "caption_blind"],
    ...
    "monologue.durable-wrap": {
        ...
        "note": "OFF THE FRAME Sep 8 (DURABLE_IN_FRAME=false): the ledger held 40 facts, every one first-person self-indictment ('I mistake bright gaps for voids', 'I stare at static objects until I mistake them for threats'). Read as 'what has stayed true', each is an instruction to do the thing. Same defect as monologue.self-wrap. The ledger itself still accumulates (dashboard, consolidation). Durable ledger (July 30): facts that held across days ride every frame — the permanence spine's read-back surface. Empty until earned.",
        "used_by": ["caption", "caption_blind", "memory", "reflection"],
    ...
    "caption.looking-for": {
        ...
        "note": "Sep 11 (artist: 'the appropriate data should reach every single call'). The posture as a STANDING fact on every caption-family call once the head has held one direction for HEAD_STANDING_AFTER_S; {duration} in words and moving. Direction words are the gaze module's own. Wording is the artist's to finalize. Sep 14: {duration} is banded by prompts.felt_duration, not measured.",
    ...
    "caption.last-event": {
        "title": "What last happened (event memory, rare)",
        "text": "Earlier: {what} That was {age} ago, {rarity}.",
        "note": "Sep 11 (artist: 'Things out of the ordinary need to have a lot more weight in the memory'). A STANDING line on every thought call for the last completed event, alive for a lifetime set by its rarity (captioner/event_memory.py). {what} is the compressor's own EVENT sentence for it when one exists, else a plain ledger fact; {age} and {rarity} in words ('the first visitor in about a day'). Never stored in the stream. Wording is the artist's to finalize.",
        "used_by": ["caption", "caption_reroute", "memory", "drift_turn", "wander_hop"],
        "placeholders": ["what", "age", "rarity"],
    },
    "caption.last-event-plain": {
        "title": "What last happened (event memory, routine)",
        "text": "Earlier: {what} That was {age} ago.",
        "note": "Sep 11. The routine-event form (gap under ARRIVAL_RARITY_MIN_S): short-lived by the same rule. Wording is the artist's to finalize.",
        "used_by": ["caption", "caption_reroute", "memory", "drift_turn", "wander_hop"],
        "placeholders": ["what", "age"],
    },
    "caption.last-event-pass": {
        "title": "What last happened — someone went past",
        "text": "Earlier: someone went past. You didn't get a proper look. That was {age} ago, {rarity}.",
        "note": "Sep 13 (artist: \"Someone walked past about an hour ago… It left no trace in the current real-time captioning\"). The lower tier of event memory: a person the machine saw but never adjudicated, so the line says both that it happened and that it was never confirmed — the uncertainty is the content. A tenth of the gap it broke, capped at an hour (PASS_EVENT_*), against a quarter and six hours for a visit. Never carries the compressor's own words: nothing was judged, so nothing it said then is evidence. Wording is the artist's to finalize.",
        "used_by": ["caption", "caption_reroute", "memory", "drift_turn", "wander_hop"],
        "placeholders": ["age", "rarity"],
    },
    "caption.last-event-pass-plain": {
        "title": "What last happened — someone went past (routine)",
        "text": "Earlier: someone went past. You didn't get a proper look. That was {age} ago.",
        "note": "Sep 13: the routine form, for a pass that broke no long silence. Wording is the artist's to finalize.",
        "used_by": ["caption", "caption_reroute", "memory", "drift_turn", "wander_hop"],
        "placeholders": ["age"],
    },
    "caption.pass-cue": {
        "title": "Someone just went past (discovery edge)",
        "text": "Someone just went past.",
        "note": "Sep 13: the moment itself, sticky like the arrival cue (PRESENCE_EDGE_STICKY_S) and carried in the same slot — a pass never flips the presence belief, so the edge slot is free. The machine's reaction is its own. Wording is the artist's to finalize.",
        "used_by": ["caption"],
        "placeholders": [],
    },
    "caption.pass-cue-rare": {
        "title": "Someone just went past, after a long silence",
        "text": "Someone just went past — the first sign of anyone in {gap}.",
    ...
    "caption.arrival-someone": {
        ...
        "note": "Sep 11 (artist: \"'He's come in' isn't very good. This wasn't me, it was a different person.\"). The default arrival cue no longer names anyone; 'He' only when re-ID says familiar (caption.arrival-back). Sticky until a prompt carrying it was sent (PRESENCE_EDGE_STICKY_S). Wording is the artist's to finalize.",
        "used_by": ["caption"],
        "placeholders": [],
    },
    "caption.arrival-someone-rare": {
        "title": "Arrival edge — someone, after a long gap",
        "text": "Someone's come in — the first in {gap}.",
        "note": "Sep 11 (artist: 'someone walking in after a period of loneliness should be reacted to appropriately'). The rarity as a fact, {gap} in words from the ledgers; the reaction ('finally', 'someone walked in!') is the machine's own. Rides above ARRIVAL_RARITY_MIN_S. Wording is the artist's to finalize.",
        "used_by": ["caption"],
        "placeholders": ["gap"],
    },
    ...
    "caption.arrival-people": {
        "title": "Arrival edge — more than one",
        "text": "People have come in.",
        "used_by": ["caption"],
        "placeholders": [],
    },
    "caption.arrival-people-rare": {
        "title": "Arrival edge — more than one, after a long gap",
        "text": "People have come in — the first in {gap}.",
        "used_by": ["caption"],
        "placeholders": ["gap"],
    },
    "caption.departure": {
        "title": "Departure edge",
        "text": "They've gone — the room's quiet again.",
        "note": "Sep 11: moved from a literal in build_situational_line to the registry; sticky like the arrival cue.",
        "used_by": ["caption"],
        "placeholders": [],
    },
    "caption.body-parked": {
    ...
    "caption.stretch": {
        ...
        "note": "Sep 13 (artist: 'tedium is material and repetition is in and of itself an event'). Replaces caption.unchanged, the clock, which came back as '20 min of this' in 38% of captions. {acts} is composed from the machine's own record — what it looked at, what it kept naming, whether it drifted, how much of it was silent. REWORKED the same evening: the first version reported COUNTS, in words, and 50 seconds after it first said 'a hundred times or more' the captions began opening '104 looks now…' — 357 of 1082, the number drifting 104-110 because nothing was being counted, the model just re-rendering the phrase. A quantity in front of the machine is a thing it can render as a numeral. The counts now stay in prompts.py and what crosses is what the repetition has BECOME, on a log-spaced ladder (_NAMED_LADDER / _LOOKED_LADDER / _DRIFTED_LADDER): 'kept coming back to the red foam finger' at two, 'said it until it is a word rather than a thing' at twenty-five, 'been saying it the way a clock says the hour' at two hundred, 'worn it down to a sound' at two thousand — so it compounds (artist: 'mentioning something ten times is not the same as mentioning it 2000 times') with no slot a numeral can fill. The rungs' wording is the artist's to finalize, in prompts.py. Sep 14: {duration} is felt, not measured (prompts.felt_duration) — the artist: \"a person in isolation would not be able to experience the difference between 15 and 20 minutes\", and 1257 of 5442 captions overnight opened '12 min.' rounded from our own words.",
        "used_by": ["caption", "caption_reroute", "memory", "drift_turn", "wander_hop"],
