...
# Import hand controller from parent servo_control directory
try:
    import sys
    import os
    sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'servo_control'))
...
class CleanCursorInterface:
    ...
    def start_emotion_transition(self, target_emotion):
        ...
        # Get available datasets for target emotion
        available_datasets = self.get_available_datasets_for_emotion(target_emotion)
        if available_datasets:
            target_dataset = available_datasets[0]  # Start with first available
    ...
    def refresh_datasets(self):
        """Refresh the available datasets (placeholder for now)."""
        print("🔄 Refreshing datasets... (placeholder implementation)")
        # This would normally scan the dataset directory and populate self.available_datasets
        pass
...
"""
    ...
    def step_markov_generation(self):
        ...
        try:
            ...
            try:
                # Simple weighted choice without dramatic jumps - probability flattening prevents sinks
