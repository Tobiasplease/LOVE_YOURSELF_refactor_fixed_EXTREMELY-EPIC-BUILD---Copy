            ...
            for component, expected_interval in self.expected_intervals.items():
                ...
                if time_since_heartbeat > expected_interval * 3:  # 3x the expected interval
                    if last_heartbeat > 0:  # Only alert if we've seen it before
                        ...
                        log_json_entry(LogType.ERROR, {
                            "message": f"Component '{component}' has gone silent",
                            "component": component,
                            "silent_failure_data": silence_data
                        })
...
def log_silent_failure(component: str, expected_action: str, context: str = ""):
    ...
    log_json_entry(LogType.ERROR, {
        "message": f"Silent failure detected: {component} failed to {expected_action}",
        "component": component,
        "silent_failure": failure_data
    })
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                }.get(operation_name, 1000)  # Default 1s
