...
try:
    ...
    from grbl.gcode_segmenter import GCodeSegment, GCodeSegmenter
    from grbl.grbl_utils import DEFAULT_CMD_TIMEOUT, DEFAULT_MOVE_TIMEOUT, send_cmd, wait_until_idle
...
class SegmentedExecutor:
    ...
    def execute_gcode_file_segmented(self, ser, gcode_file: str, move_timeout=DEFAULT_MOVE_TIMEOUT) -> bool:
        ...
        try:
            ...
            if not segments:
                log_json_entry(
                    LogType.ERROR, {"action": "no_segments_found", "file": gcode_file}, print_message="[❌] No segments found in G-code file"
                )
                return False
    ...
    def _fallback_to_original_execution(self, ser, gcode_file: str, move_timeout: float) -> bool:
        ...
        log_json_entry(
            LogType.GRBL, {"action": "fallback_execution_start", "file": gcode_file}, print_message="[⚠️] Using fallback execution method..."
        )
        ...
        try:
            ...
            log_json_entry(
                LogType.ERROR, {"action": "fallback_execution_failed", "error": str(e)}, print_message=f"[❌] Fallback execution also failed: {e}"
            )
            return False
    ...
    def _check_person_detection_pause(self):
        ...
        try:
            ...
            if person_present and not self.is_paused_for_person:
                ...
                log_json_entry(
                    LogType.GRBL,
                    {"action": "drawing_paused_for_person", "segment": self.current_segment, "total": self.total_segments, "person_detected": True},
            ...
            log_json_entry(
                LogType.ERROR, {"action": "person_detection_check_failed", "error": str(e)}, print_message=f"[⚠️] Person detection check failed: {e}"
            )
