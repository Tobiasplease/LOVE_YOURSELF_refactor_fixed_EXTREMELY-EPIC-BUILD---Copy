...
MIND_POSITION_TTL_S = int(os.getenv("MIND_POSITION_TTL_S", 86400))  # Sep 8: a conclusion about a THING is world knowledge, not a mood — it should outlast the half hour it used to
...
MOOD_FATIGUE_PER_H = float(os.getenv("MOOD_FATIGUE_PER_H", 0.02))  # arousal down per hour awake (cap 8 h) — a pressure, not a verdict
MOOD_ALONE_PER_H = float(os.getenv("MOOD_ALONE_PER_H", 0.015))  # valence down per hour alone
MOOD_STILL_PER_H = float(os.getenv("MOOD_STILL_PER_H", 0.02))  # arousal down per hour unchanged (cap 4 h)
...
MIND_NOBODY_EVERY_S = float(os.getenv("MIND_NOBODY_EVERY_S", 60))  # the empty-room fact rides at most this often while nobody is there
MIND_SETTLED_MIN_WORDS = int(os.getenv("MIND_SETTLED_MIN_WORDS", 5))  # a fragment ("A tool?") does not replace a real conclusion
MIND_SETTLED_IN_CUE = int(os.getenv("MIND_SETTLED_IN_CUE", 2))  # settled conclusions named on a look, for the things in view
