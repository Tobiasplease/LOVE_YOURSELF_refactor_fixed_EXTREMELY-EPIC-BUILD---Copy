...
def main():
    ...
    def _find_closest_servo_state(self, target_state_key, servo_transitions, discretization):
        ...
        if 'transitions' in chain:
            # Old format
            transitions = chain['transitions']
        elif 'cursor_transitions' in chain:
            # New enhanced format - use cursor transitions for generation
            transitions = chain['cursor_transitions']
        else:
            print(f"❌ No valid transitions in Markov chain for {self.current_emotional_state}")
            return
        ...
        candidate_state_str = str(candidate_state)  # Convert to string for lookup
        
        if candidate_state_str in transitions:
            self.current_markov_state = candidate_state_str
        else:
            # Find closest existing state - PARSE STRING KEYS!
            min_dist = float('inf')
            best_state = None
            for state_key in transitions.keys():
                try:
                    # Parse the string key back to coordinates
                    parsed_coords = self.parse_markov_state_key(state_key)
                    if parsed_coords and len(parsed_coords) >= 2:
                        state_x, state_y = int(parsed_coords[0]), int(parsed_coords[1])
                        dist = ((state_x - current_grid_x) ** 2 + (state_y - current_grid_y) ** 2) ** 0.5
                        if dist < min_dist:
                            min_dist = dist
                            best_state = state_key
                except (ValueError, TypeError) as e:
                    print(f"⚠️ Skipping invalid state key '{state_key}': {e}")
                    continue
    ...
    def step_markov_generation(self):
        ...
        # Convert current state from string back to tuple if needed
        if isinstance(self.current_markov_state, str):
            try:
                # Parse string like "(60, 45)" back to tuple (60, 45)
                parsed = self.parse_markov_state_key(self.current_markov_state)
        ...
        # Get current state key as string for lookup
        ...
        next_states = list(next_states_dict.keys())
        probabilities = [next_states_dict[state] for state in next_states]
        ...
        # Parse next state
        try:
            parsed = self.parse_markov_state_key(next_state_key)
            if parsed and len(parsed) >= 2:
