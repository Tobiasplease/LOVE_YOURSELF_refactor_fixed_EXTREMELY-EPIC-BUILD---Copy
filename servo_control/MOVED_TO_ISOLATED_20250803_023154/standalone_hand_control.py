...
class CleanCursorInterface:
    ...
    def __init__(self):
        ...
        # Setup optional mood data integration
        self.mood_socket = None
        self.mood_listener_active = False
        self.last_received_mood = None
        self.setup_mood_listener()
    ...
    # === MOOD DATA INTEGRATION (Optional) ===
    def setup_mood_listener(self):
        """Setup optional mood data listener for machine.py integration."""
        try:
            # Try to create a simple UDP listener for mood data
            self.mood_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            self.mood_socket.bind(('localhost', 12345))  # Simple local port
            self.mood_socket.settimeout(0.1)  # Non-blocking
            
            # Start listener thread
            self.mood_listener_active = True
            mood_thread = threading.Thread(target=self.mood_listener_loop, daemon=True)
            mood_thread.start()
            
            print("🔗 Mood data listener active on localhost:12345")
            print("💡 Can receive emotional state updates from machine.py")
            
        except Exception as e:
            print(f"📡 Mood listener setup failed (running standalone): {e}")
            self.mood_socket = None
    
    def mood_listener_loop(self):
        """Background thread to listen for mood data from machine.py."""
        while self.mood_listener_active:
            try:
                if self.mood_socket:
                    data, addr = self.mood_socket.recvfrom(1024)
                    mood_data = json.loads(data.decode())
                    self.handle_mood_update(mood_data)
            except socket.timeout:
                continue  # Normal timeout, keep listening
            except Exception as e:
                if self.mood_listener_active:  # Only print if we're still supposed to be listening
                    print(f"📡 Mood listener error: {e}")
                break
    
    def handle_mood_update(self, mood_data):
        """Handle incoming mood data from machine.py."""
        try:
            new_emotion = mood_data.get('emotional_state', '')
            if new_emotion and new_emotion in self.emotional_states:
                if new_emotion != self.current_emotional_state:
                    print(f"📡 Received mood update: {new_emotion}")
                    self.switch_emotional_state(new_emotion)
                    self.last_received_mood = mood_data
                    
        except Exception as e:
            print(f"❌ Error handling mood update: {e}")
    
    def on_closing(self):
        """Clean shutdown of the application."""
        print("🚪 Shutting down standalone hand control...")
        
        # Stop mood listener
        if self.mood_listener_active:
            self.mood_listener_active = False
            print("📡 Stopped mood data listener")
        
        # Close mood socket
        if self.mood_socket:
            try:
                self.mood_socket.close()
                print("🔌 Closed mood data socket")
            except:
                pass
        ...
        if hasattr(self, 'recording') and self.recording:
            self.stop_recording()
        if hasattr(self, 'playing_back') and self.playing_back:
            self.stop_playback()
    ...
    def setup_ui(self):
        ...
        dataset_frame = ttk.LabelFrame(emotion_inner, text="📊 Recorded Movements")
        ...
        # Simple emotion-specific recording info
        self.recording_info_label = ttk.Label(dataset_inner, 
                                            text=f"📁 {self.current_emotional_state.title()}: No recordings yet", 
                                            font=("Arial", 11, "bold"), foreground="#8B5CF6")
        self.recording_info_label.pack(anchor=tk.W, pady=(0, 10))
        
        # Simple management buttons
        management_btn_frame = ttk.Frame(dataset_inner) 
        management_btn_frame.pack(fill=tk.X, pady=(0, 5))
        
        # Clear current emotion button
        ttk.Button(management_btn_frame, text="�️ Clear This Emotion", 
                  command=self.clear_current_emotion, width=20).pack(side=tk.LEFT, padx=(0, 10))
        
        # Clear all emotions button  
        ttk.Button(management_btn_frame, text="🧹 Clear All Emotions", 
                  command=self.clear_all_emotions, width=20).pack(side=tk.LEFT)
    ...
    def switch_emotional_state(self, emotion_name):
        ...
        # Update the simplified recording info display
    ...
    def clear_current_emotion(self):
        """Clear all recordings for the current emotion"""
        if self.current_emotional_state in self.datasets:
            del self.datasets[self.current_emotional_state]
            self.update_recording_info()
            print(f"🗑️ Cleared all recordings for {self.current_emotional_state}")
    
    def clear_all_emotions(self):
        """Clear all recordings for all emotions"""
        from tkinter import messagebox
        if messagebox.askyesno("Clear All", "Clear ALL emotion recordings?"):
            self.datasets.clear()
            self.update_recording_info()
            print("🧹 Cleared all emotion recordings")
    
    def update_recording_info(self):
        """Update the simple recording info display"""
        emotion_data = self.datasets.get(self.current_emotional_state, [])
        total_points = sum(len(recording) for recording in emotion_data)
        
        if emotion_data:
            info_text = f"📁 {self.current_emotional_state.title()}: {len(emotion_data)} recordings, {total_points} points"
        else:
            info_text = f"📁 {self.current_emotional_state.title()}: No recordings yet"
            
        self.recording_info_label.config(text=info_text)
    ...
    def stop_recording(self):
        ...
        # Save to simplified dataset structure
        self.save_recording()
    ...
    def save_recording(self):
        """Save servo-based recorded movements and update simplified dataset tracking."""
        emotion = self.current_emotional_state
        if emotion not in self.recorded_movements or len(self.recorded_movements[emotion]) < 2:
            print("❌ No servo data to save")
            return
        
        # Simply add to the datasets structure for this emotion
        if emotion not in self.datasets:
            self.datasets[emotion] = []
        
        # Store the recording data directly
        movements = self.recorded_movements[emotion]
        markov_chain = self.markov_chains.get(emotion, {})
        
        self.datasets[emotion].append({
            'movements': movements,
        ...
        print(f"💾 Saved recording for {emotion}: {len(movements)} points")
        print(f"📊 Total recordings for {emotion}: {len(self.datasets[emotion])}")
        
        # Clear the recorded movements for next session
        self.recorded_movements[emotion] = []
...
"""
    ...
    def step_markov_generation(self):
        ...
        try:
            ...
            flattening_factor = 0.7  # Makes distribution more uniform (0.5 = uniform, 1.0 = original)
            ...
            try:
                # Add small chance to pick a completely random state to avoid getting stuck
                if random.random() < 0.05:  # 5% chance of random jump for diversity
                    available_states = list(transitions.keys())
                    next_state_key = random.choice(available_states)
                    self.current_markov_state = next_state_key
                    print(f"🎲 Diversity jump to break out of probability sink: {next_state_key}")
                else:
            ...
            if len(next_state) >= 4:  # Should be (servo1, servo2, servo3, servo4)
                # GOLDEN MASTER STYLE: Simple easing (not complex adaptive easing)
                easing_factor = self.generation_easing_factor  # Use simple fixed easing
                ...
                easing_factor = self.generation_easing_factor
