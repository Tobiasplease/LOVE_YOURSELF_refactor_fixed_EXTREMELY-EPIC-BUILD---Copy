...
class CleanCursorInterface:
    ...
    def __init__(self):
        ...
        self.record_interval = 0.05  # 20 Hz - captures both movement and stillness
        ...
        self.generation_speed = 0.1  # How fast to generate (seconds between steps)
    ...
    def setup_ui(self):
        ...
        self.record_status = ttk.Label(record_frame, text="Ready to record (Spacebar)", foreground="gray")
        self.record_status.pack(side=tk.LEFT, padx=10)
        
        # Markov status display
        self.markov_status = ttk.Label(record_frame, text="No chains built", foreground="gray", font=("Arial", 8))
    ...
    def on_mouse_move(self, event):
        ...
        # FIXED CANVAS DIMENSIONS - No more dynamic sizing!
        canvas_width = 480  # Fixed width - matches initialize values
        canvas_height = 200  # Fixed height - matches initialize values
        
        if canvas_width > 1 and canvas_height > 1:
            # Always track raw mouse position for thaw transitions
            raw_mouse_x = event.x / canvas_width
            raw_mouse_y = event.y / canvas_height
            ...
            if self.move_count < 5:
                ...
                print(f"🎯 Mouse move {self.move_count}: ({self.mouse_x:.3f}, {self.mouse_y:.3f}) canvas: {canvas_width}x{canvas_height}{freeze_status}")
    ...
    def start_recording(self):
        ...
        print(f"⏰ Sampling at {1/self.record_interval:.0f} Hz (captures stillness!)")
    ...
    def record_position_sample(self):
        ...
        # Create position state for Markov chain
        # Discretize position into grid for Markov states
        grid_size = 20  # 20x20 grid = 400 possible states
        ...
        if len(self.recorded_positions) <= 5:
            print(f"📍 Sample {len(self.recorded_positions)}: ({self.mouse_x:.3f}, {self.mouse_y:.3f}) -> grid ({grid_x}, {grid_y})")
    ...
    def build_markov_chain(self):
        ...
        print(f"🎯 Chain covers {unique_states}/{20*20} possible grid positions ({100*unique_states/400:.1f}%)")
    ...
    def start_markov_generation(self):
        ...
        grid_size = chain.get('grid_size', 20)
    ...
    def step_markov_generation(self):
        ...
        # Smooth interpolation to new position (optional - makes movement less jerky)
        smooth_factor = 0.3  # How much to smooth the movement
        self.mouse_x = self.mouse_x * (1 - smooth_factor) + new_x * smooth_factor
        self.mouse_y = self.mouse_y * (1 - smooth_factor) + new_y * smooth_factor
