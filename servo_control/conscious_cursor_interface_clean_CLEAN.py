...
Focus: Working cursorÔåÆservo control + simple emotional switching.

Core Features:
- Direct cursorÔåÆ4 servo control (WORKING)
...
try:
    ...
    print("Ô£à Hand controller available")
except ImportError:
    print("ÔÜá´©Å Hand controller not available - simulation mode")
...
class CleanCursorInterface:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("­ƒÄ» Clean Emotional Hand Control - Baseline")
        ...
        print(f"­ƒÄ» Initialized finger positions: {self.finger_positions}")
        print(f"­ƒÄ» Initialized finger targets: {self.finger_targets}")
        ...
        print("­ƒÄ» Clean Emotional Hand Control initialized")
        print("­ƒÄ« Direct wave-based cursorÔåÆservo control ready")
        print("­ƒÿè 5 emotional states available for testing")
        print(f"­ƒôÉ FIXED canvas dimensions: 480x200 (no more resizing)")
        print(f"­ƒÄ» Condensed control area: 25%-75% of canvas width for precise movement")
    ...
    def setup_ui(self):
        ...
        # === CONNECTION FRAME ===
        conn_frame = ttk.LabelFrame(self.scrollable_frame, text="­ƒöî Connection")
        ...
        self.status_label = ttk.Label(conn_frame, text="ÔØî Disconnected", width=20)
        self.status_label.pack(side=tk.LEFT, padx=(10, 0))
        
        # === EMOTIONAL STATE CONTROL ===
        emotion_frame = ttk.LabelFrame(self.scrollable_frame, text="­ƒÿè Emotional State Control")
        ...
        self.record_btn = ttk.Button(record_frame, text="­ƒÄ¼ Record Movement (2min)", 
                                   command=self.start_recording, width=22)  # Increased to accommodate longer text
        self.record_btn.pack(side=tk.LEFT, padx=5)
        
        # Playback button
        self.playback_btn = ttk.Button(record_frame, text="ÔûÂ´©Å Play Back", 
        ...
        self.generate_btn = ttk.Button(record_frame, text="­ƒºá Generate (Markov)", 
        ...
        control_frame = ttk.LabelFrame(self.scrollable_frame, text="­ƒÄ» Hand Control Area")
        ...
        mode_frame = ttk.LabelFrame(self.scrollable_frame, text="­ƒÄø´©Å Control Modes")
        mode_frame.pack(fill=tk.X, padx=10, pady=5)
        
        reverse_cb = ttk.Checkbutton(mode_frame, text="­ƒöä Reverse Vertical", 
                                   variable=self.reverse_vertical)
        reverse_cb.pack(side=tk.LEFT, padx=5)
        
        # Person detection simulation toggle
        person_cb = ttk.Checkbutton(mode_frame, text="­ƒæñ Person Detected (Sim)", 
                                  variable=self.person_detected,
                                  command=self.on_person_detection_toggle)
        person_cb.pack(side=tk.LEFT, padx=10)
        
        reset_btn = ttk.Button(mode_frame, text="­ƒÄ» Reset to Center", 
                             command=self.reset_to_center)
        reset_btn.pack(side=tk.RIGHT, padx=5)
        
        # === FREEZE BEHAVIOR CONTROLS ===
        freeze_frame = ttk.LabelFrame(self.scrollable_frame, text="ÔØä´©Å Freeze Behavior")
        ...
        wave_frame = ttk.LabelFrame(self.scrollable_frame, text="­ƒîè Wave Control Parameters")
    ...
    def switch_emotional_state(self, emotion_name):
        ...
        print(f"­ƒÄ¡ Switched to {emotion_name} emotional state")
        print(f"­ƒôè Parameters: sensitivity={params['cursor_sensitivity']:.1f}")
    
    def toggle_connection(self):
        """Toggle hand controller connection - FIXED LOGIC."""
        if not HAND_CONTROLLER_AVAILABLE:
            self.status_label.config(text="ÔØî Controller unavailable")
            print("ÔÜá´©Å Hand controller not available - simulation mode")
            return
        ...
        if not self.connected:
            try:
                ...
                if self.hand_controller.serial_connection:
                    ...
                    self.connected = True
                    self.status_label.config(text="Ô£à Connected")
                    self.connect_btn.config(text="Disconnect")
                    print("Ô£à Connected to hand controller on COM3")
                    print("­ƒÄ« Manual override enabled - ready for cursor control")
                else:
                    self.status_label.config(text="ÔØî Connection failed")
                    print("ÔØî Failed to connect to COM3")
            except Exception as e:
                self.status_label.config(text="ÔØî Connection error")
                print(f"ÔØî Connection error: {e}")
        else:
            # Disconnect
            ...
            self.connected = False
            self.status_label.config(text="ÔØî Disconnected")
            self.connect_btn.config(text="Connect to Hand Controller")
            print("­ƒöî Disconnected from hand controller")
    ...
    def on_canvas_configure(self, event):
        ...
        if event.width != self.canvas_width or event.height != self.canvas_height:
            ...
            print(f"­ƒöÆ Canvas size locked: {self.canvas_width}x{self.canvas_height}")
    ...
    def on_mouse_move(self, event):
        ...
        if self.move_count < 5:
            ...
            print(f"­ƒÄ» Mouse move {self.move_count}: ({self.mouse_x:.3f}, {self.mouse_y:.3f}) canvas: {canvas_width}x{canvas_height} event: ({event.x}, {event.y}){freeze_status}")
    ...
    def on_spacebar_press(self, event):
        ...
        if self.recording:
            self.stop_recording()
            print("ÔÅ╣´©Å Spacebar: Stopped recording")
        else:
            self.start_recording()
            print("­ƒÄ¼ Spacebar: Started recording")
    ...
    def on_person_detection_toggle(self):
        ...
        if self.person_detected.get():
            ...
            if current_time - self.last_detection_time < self.detection_cooldown:
                ...
                print(f"­ƒæñ Person detection on cooldown - {remaining:.1f}s remaining")
    ...
    def trigger_freeze(self):
        ...
        print(f"ÔØä´©Å FREEZE triggered for {self.freeze_duration:.1f}s at position ({self.mouse_x:.2f}, {self.mouse_y:.2f})")
    ...
    def start_thaw(self):
        ...
        print(f"­ƒöä Starting smooth thaw transition back to {self.current_emotional_state}")
    ...
    def end_freeze(self):
        ...
        print(f"Ô£à Freeze complete - resumed normal {self.current_emotional_state} movement")
    ...
    def reset_to_center(self):
        ...
        print("­ƒÄ» Reset to center position")
    
    def start_control_loop(self):
        """Start the main control loop - clean and direct."""
        self.running = True
        print("­ƒÄ» Starting direct control loop...")
        self.control_loop()
    ...
    def control_loop(self):
        ...
        if self.loop_count < 5:
            print(f"­ƒöä Control loop {self.loop_count}: dt={dt:.3f}, mouse=({self.mouse_x:.3f}, {self.mouse_y:.3f})")
        ...
        if self.direct_count < 5 or self.direct_count % 30 == 0:
            ...
            print(f"­ƒÄ» Direct control {self.direct_count}: positions={[f'{p:.1f}' for p in self.finger_positions]} from targets={[f'{t:.1f}' for t in self.finger_targets]}{freeze_status}")
    ...
    def calculate_finger_targets(self):
        ...
        if self.calc_count < 5 or (targets_changed and self.calc_count % 30 == 0):
            print(f"­ƒÄ» Targets {self.calc_count}: {[f'{t:.1f}' for t in self.finger_targets]} (mouse: {self.mouse_x:.3f}, {self.mouse_y:.3f})")
    ...
    def update_canvas(self):
        ...
        if self.is_frozen:
            if self.is_thawing:
                ...
                thaw_progress = min(1.0, thaw_elapsed / self.thaw_duration)
                self.canvas.create_text(canvas_width-10, 10, text=f"­ƒöä THAWING ({thaw_progress*100:.0f}%)", 
                ...
                if remaining_freeze > 0:
                    self.canvas.create_text(canvas_width-10, 10, text=f"ÔØä´©Å FROZEN ({remaining_freeze:.1f}s)", 
                                          fill="cyan", anchor="ne", font=("Arial", 14, "bold"))
        elif self.recording:
            self.canvas.create_text(canvas_width-10, 10, text="­ƒö┤ RECORDING", fill="red", anchor="ne",
                                  font=("Arial", 14, "bold"))
        elif self.playing_back:
            self.canvas.create_text(canvas_width-10, 10, text="ÔûÂ´©Å PLAYING", fill="blue", anchor="ne",
                                  font=("Arial", 14, "bold"))
        elif self.generating:
            self.canvas.create_text(canvas_width-10, 10, text="­ƒÄ¿ LEARNING", fill="purple", anchor="ne",
                                  font=("Arial", 14, "bold"))
        ...
        if changed or current_time - getattr(self, 'last_any_send_time', 0) > 1.0:
            try:
                ...
                if self.send_count < 5 or self.send_count % 20 == 0:
                    print(f"­ƒôñ Sending {self.send_count}: positions={positions} from finger_positions={[f'{p:.1f}' for p in self.finger_positions]}")
                ...
                print(f"ÔØî Error sending to hand controller: {e}")
                import traceback
                traceback.print_exc()
    ...
    def start_recording(self):
        ...
        self.record_btn.config(text="ÔÅ╣´©Å Stop Recording")
        ...
        print(f"­ƒÄ¼ Started TIME-BASED recording for {self.current_emotional_state}")
        print(f"ÔÅ░ Sampling at {1/self.record_interval:.0f} Hz (captures easing motions!)")
    ...
    def record_position_sample(self):
        ...
        if len(self.recorded_positions) <= 5:
            print(f"­ƒôì Sample {len(self.recorded_positions)}: ({self.mouse_x:.3f}, {self.mouse_y:.3f}) -> grid ({grid_x}, {grid_y}) [80x80 grid] speed: {math.sqrt(velocity_x**2 + velocity_y**2):.3f}")
    ...
    def stop_recording(self):
        ...
        self.record_btn.config(text="­ƒÄ¼ Record Movement (45s)")
        self.record_status.config(text=f"Recorded {sample_count} samples in {duration:.1f}s", foreground="green")
        
        print(f"­ƒÄ¼ Stopped recording. Captured {sample_count} position samples in {duration:.1f} seconds")
        print(f"­ƒôè Sample rate: {sample_count/duration:.1f} Hz")
    ...
    def build_markov_chain(self):
        ...
        if len(self.recorded_positions) < 2:
            print("ÔÜá´©Å Not enough samples to build Markov chain")
            return
            
        print(f"­ƒöù Building Markov chain from {len(self.recorded_positions)} samples...")
        ...
        print(f"Ô£à Built Markov chain: {unique_states} unique states, {avg_transitions:.1f} average transitions per state")
        print(f"­ƒÄ» Chain covers {unique_states}/{80*80} possible grid positions ({100*unique_states/6400:.1f}%)")
    ...
    def save_recording(self):
        ...
        print(f"­ƒÆ¥ Saved recording with Markov chain to {filename}")
        print(f"­ƒôê Chain has {markov_chain.get('unique_states', 0)} states from {len(positions)} samples")
    ...
    def post_process_rhythm_analysis(self):
        ...
        print(f"­ƒÄÁ ENHANCED: Analyzing temporal rhythm patterns in {len(vectors)} vectors...")
        ...
        print(f"Ô£à Enhanced temporal rhythm analysis complete - stillness and movement patterns detected")
    ...
    def detect_stillness_patterns(self, vectors):
        ...
        print(f"   ­ƒôì Detected stillness patterns: {sum(1 for v in vectors if v.get('position_hold', False))} position holds")
        print(f"   ­ƒñÅ Detected micro-movements: {sum(1 for v in vectors if v.get('micro_tremor', False))} micro tremors")
        print(f"   ÔÜí Detected movement impulses: {sum(1 for v in vectors if v.get('movement_impulse', False))} impulses")
        ...
        if len(movement_moments) > 3:
            ...
            if intervals:
                ...
                if len(consistent_intervals) > len(intervals) * 0.6:
                    print(f"   ­ƒÄÁ Detected rhythmic timing: {avg_interval:.2f}s average interval")
    ...
    def detect_breathing_patterns(self, vectors):
        ...
        if center_returns:
            print(f"   ­ƒ½ü Detected breathing-like patterns: {len(center_returns)} expansion/contraction cycles")
    ...
    def load_saved_movements(self):
        ...
        if not os.path.exists("movement_recordings"):
            print("­ƒôü No saved movements found - starting fresh")
            return
        ...
        for emotion_key in self.emotional_states.keys():
            ...
            try:
                ...
                if files:
                    ...
                    print(f"­ƒôé Loaded {len(data.get('movements', []))} movements for {emotion_key}")
            except Exception as e:
                print(f"ÔÜá´©Å Error loading {emotion_key}: {e}")
        
        if loaded_count > 0:
            print(f"Ô£à Loaded movements for {loaded_count} emotional states")
        else:
            print("­ƒôü No valid saved movements found")
    ...
    def save_recording(self):
        ...
        print(f"­ƒÆ¥ Saved recording with {len(vectors)} vectors to {filename}")
    ...
    def start_playback(self):
        ...
        self.playback_btn.config(text="ÔÅ╣´©Å Stop Playback")
        self.record_status.config(text=f"Playing back {self.current_emotional_state}...", foreground="blue")
        
        print(f"ÔûÂ´©Å Started playback of {len(self.current_playback)} movements for {self.current_emotional_state}")
    
    def stop_playback(self):
        """Stop playback."""
        self.playing_back = False
        self.playback_btn.config(text="ÔûÂ´©Å Play Back")
        self.record_status.config(text="Playback stopped", foreground="gray")
        print("ÔÅ╣´©Å Playback stopped")
    ...
    def start_markov_generation(self):
        ...
        if self.current_emotional_state not in self.markov_chains:
            print(f"ÔØî No Markov chain available for {self.current_emotional_state}")
        ...
        if not chain.get('transitions'):
            print(f"ÔØî Empty Markov chain for {self.current_emotional_state}")
            return
        ...
        self.generate_btn.config(text="ÔÅ╣´©Å Stop Generation")
        self.markov_status.config(text="Generating movement...", foreground="purple")
        
        print(f"­ƒÄ¿ Started Markov generation for {self.current_emotional_state}")
        print(f"­ƒÄ» Starting from state {start_state} -> position ({self.mouse_x:.3f}, {self.mouse_y:.3f})")
        print(f"­ƒôè Chain has {len(transitions)} states available")
    ...
    def step_markov_generation(self):
        ...
        if self.current_markov_state not in transitions:
            ...
            print(f"­ƒöä Dead end reached, jumping to {self.current_markov_state}")
            return
    ...
    def stop_markov_generation(self):
        ...
        self.generate_btn.config(text="­ƒºá Generate (Markov)")
        self.markov_status.config(text="Generation stopped", foreground="gray")
        
        print(f"­ƒÄ¿ Stopped Markov generation after {duration:.1f} seconds")
    ...
    def load_saved_movements(self):
        ...
        if not os.path.exists("movement_recordings"):
            print("­ƒôü No movement_recordings directory found")
            return
        ...
        for emotion_key in self.emotional_states.keys():
            ...
            try:
                ...
                if 'markov_chain' in data and data['markov_chain']:
                    ...
                    chain_count += 1
                    print(f"­ƒöù Loaded Markov chain for {emotion_key}: {data['markov_chain'].get('unique_states', 0)} states")
                    
            except Exception as e:
                print(f"ÔØî Error loading {latest_file}: {e}")
                continue
        
        if loaded_count > 0:
            print(f"Ô£à Loaded {loaded_count} movement recordings and {chain_count} Markov chains")
            self.markov_status.config(text=f"{chain_count} chains loaded", foreground="green")
        else:
            print("­ƒôü No saved recordings found")
    ...
    def start_markov_generation(self):
        ...
        print(f"­ƒÄ¿ Started ENHANCED Markov generation for {self.current_emotional_state}")
        print(f"­ƒÄ» Starting from state {self.current_markov_state} (near current position)")
        print(f"ÔÜí Generation rate: {1/self.generation_speed:.1f} Hz for smooth movement")
    ...
    def step_markov_generation(self):
        ...
        if self.current_markov_state not in transitions:
            ...
            print(f"ÔÜá´©Å Dead end state {self.current_markov_state}, finding nearby state...")
            ...
            for radius in range(1, 5):
                ...
                if found_state:
                    ...
                    print(f"­ƒöä Recovered with nearby state {found_state}")
                    break
            else:
                # Last resort: pick any random state
                self.current_markov_state = random.choice(list(transitions.keys()))
                print(f"­ƒÄ▓ Jumped to random state {self.current_markov_state}")
    ...
    def stop_markov_generation(self):
        ...
        print(f"­ƒÄ¿ Stopped Markov generation after {generation_duration:.1f} seconds")
    
    def load_saved_movements(self):
        """Load previously saved movements from disk for persistence."""
        if not os.path.exists("movement_recordings"):
            print("­ƒôé No movement recordings directory found")
            return
        ...
        for emotion_key in self.emotional_states.keys():
            ...
            if files:
                ...
                try:
                    ...
                    if 'movements' in data:
                        self.recorded_movements[emotion_key] = data['movements']
                        print(f"­ƒôé Loaded {len(data['movements'])} movements for {emotion_key}")
                    
                    # Load Markov chain
                    if 'markov_chain' in data:
                        self.markov_chains[emotion_key] = data['markov_chain']
                        chain = data['markov_chain']
                        print(f"­ƒöù Loaded Markov chain for {emotion_key}: {chain.get('unique_states', 0)} states")
        ...
        if loaded_count > 0:
            ...
            print("­ƒôé No saved movements found")


def main():
    """Main function to start the interface."""
    print("­ƒÜÇ Starting Clean Emotional Hand Control...")
    ...
    try:
        ...
        if hasattr(interface, 'hand_controller') and interface.hand_controller:
            try:
                ...
                pass
        print("­ƒöî Clean shutdown complete")
