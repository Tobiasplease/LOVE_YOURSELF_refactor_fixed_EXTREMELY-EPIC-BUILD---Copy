...
check("someone-here leads the cue when believed", look2["cue"].index("Someone is here") < look2["cue"].index("You look") and "Someone is here" not in look["user"], look2["cue"])
m_seen, _ = fresh_mind()  # PRESENCE WITHIN A SECOND (Sep 7): the detector's word, no adjudication
m_seen.absorb("A thought before anyone came.", "think", "c", now - 120)
a_seen = Agent()
m_seen.build("think", now, a_seen, {}, "/tmp/x.jpg")  # the empty room first, so the arrival is an edge
seen_call = m_seen.build("look", now, a_seen, {"person_in_frame": True}, "/tmp/x.jpg")
...
c = m.build("think", now, Agent(), {}, None)
check("recall rides on the last thing it wrote, framed as memory", "You remember, from" in c["user"] or c["memory"] is None, c["user"])
_hotagent = Agent(); _hotagent._salience_hot = True
c_hot = m.build("think", now, _hotagent, {}, None)
check("salience hot strips the recall from the call", "You remember, from" not in c_hot["user"], c_hot["user"])
...
check("the last thing written is the recall query, not a prompt line", m3.last_written(now) == "Scattering.", m3.last_written(now))
check("nothing written yet: no query", fresh_mind()[0].last_written(now) == "")
...
print("\n[5h] the reflection's time spine; tempo follows the event; events as memory")
m, M = fresh_mind()
_lt = time.localtime(now); _midnight = now - (_lt.tm_hour * 3600 + _lt.tm_min * 60 + _lt.tm_sec)
m.absorb("A morning thought. It's the same loop.", "reflection", "12:50. Something settles.", max(now - 3 * 3600, _midnight + 60))
m.absorb("The night's page. I can sit in the quiet without trying to fix it.", "dream", "04:00. Something settles.", now - 10 * 3600)
cs = m.conclusions_today(now)
check("conclusions = the day's reflection endings with clocks + the page ending", any("It's the same loop." in c and "—" in c for c in cs) and any("night's page ended" in c for c in cs), cs)
pr = build_reflection_loop_prompt("What is it for?", {"hour": ["a line"], "pages": "Some pages.", "conclusions": cs, "situation": "you woke at 11:04, no one here for about 3 hours"})
check("the prompt carries the spine, the situation and the arc ask", "Earlier today you settled" in pr and "Where you are: you woke at 11:04" in pr and "what you'd do next" in pr)
check("the old 'read your record' ask does not fire with pages", "Read your record above" not in pr and "the record doesn't show" not in pr)
a = Agent(); a._salience_event = "Something just moved in front of you."
lk = m.build("look", now, a, {}, "/tmp/x.jpg")
check("a salience event marks the call hot", lk["hot"] is True, lk["hot"])
check("a quiet think is not hot", m.build("think", now, Agent(), {}, None)["hot"] is False)
lb_hot = m.life_block(now, a, hot=True)
check("hot strips the interior lines from the life block", "you'd got to" not in lb_hot and "You remember, from" not in lb_hot, lb_hot[-160:])
...
check("belief / positions stay out of the standing block by default", "settled earlier" not in lb and "got to with" not in lb)
C.MIND_LIFE_FULL = True
check("MIND_LIFE_FULL brings the belief back, marked as memory", "Something you settled earlier" in m.life_block(now, a))
...
_cap = open("captioner/captioner.py", encoding="utf-8").read()
check("the mind decides its frames with the same code the legacy path uses", "_fs = self._video_frame_set(scene)" in _cap and "def _video_frame_set" in _cap)
...
_past = ("mind.life-past", "mind.life-before", "mind.cue-recall", "mind.cue-think-memory", "mind.life-events", "mind.life-belief", "mind.life-position", "mind.life-room", "mind.life-questions")
_marks = ("you remember", "you'd got to", "comes back", "since you woke", "you settled", "you've been carrying", "from looking before", "you thought")
check("every fragment carrying past material frames it as memory", all(any(k in R.FRAGMENTS[f]["text"].lower() for k in _marks) for f in _past), [f for f in _past if not any(k in R.FRAGMENTS[f]["text"].lower() for k in _marks)])
...
m10, _ = fresh_mind()
m10.absorb("He's under that shelf again, hunched over the desk.", "think", "c", now - 60)
_c10 = m10.build("think", now, Agent(), {"person_in_frame": False}, None)
check("when nobody is there and it wrote someone, the cue corrects it first", "No one is in the room" in _c10["cue"], _c10["cue"])
check("and the correction is dosed, not chanted", "No one is in the room" not in m10.build("think", now + 5, Agent(), {}, None)["cue"])
check("the drift clause rides only when it has been writing someone", "writing as if someone were" in _c10["cue"], _c10["cue"])
m12, _ = fresh_mind()
m12.absorb("The lamp is a white hole again.", "think", "c", now - 60)
_c12 = m12.build("think", now, Agent(), {"person_in_frame": False}, None)
check("the empty-room fact rides even with no drift (a world fact, not a mirror)", "No one is in the room" in _c12["cue"] and "writing as if" not in _c12["cue"], _c12["cue"])
m13, _ = fresh_mind()
m13.absorb("Still there. Under the shelf. Just a dark shape, hunched over.", "think", "c", now - 60)
check("a phantom with no person word still counts as drift", "writing as if someone were" in m13.build("think", now, Agent(), {}, None)["cue"])
m11, _ = fresh_mind()
m11.absorb("The lamp is a white hole again.", "think", "c", now - 60)
check("the fact is a fact, so it rides whether or not it drifted", "No one is in the room" in m11.build("think", now, Agent(), {}, None)["cue"])
check("only the adjudicator licenses person-talk past the gate", "_adj_person_ts" in open("captioner/captioner.py", encoding="utf-8").read())
