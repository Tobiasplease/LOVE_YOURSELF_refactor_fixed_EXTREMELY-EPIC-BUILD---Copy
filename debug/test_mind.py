...
print("\n[5f] a gated thought still moves the premise; restated premises are stripped")
m, M = fresh_mind()
m.absorb("The pen is a black stick in the dark.", "think", "c", now - 120)
check("premise = the last kept sentence", m.premise(now) == "The pen is a black stick in the dark.")
m.note_spoken("A black stick in the dark. It's just a line that hasn't started yet.", now - 60)
check("after a refusal the premise is the spoken tail", m.premise(now) == "It's just a line that hasn't started yet.", m.premise(now))
m.note_spoken("Just a line that hasn't started. Or the end of one.", now - 30)
check("two refusals in a row: one turn with no premise", m.premise(now) == "")
check("then the premise returns", m.premise(now + 1) != "")
m.absorb("Kept again.", "think", "c", now)
check("a kept thought resets the streak", getattr(m, "_gate_streak", 0) == 0 and m.premise(now + 2).endswith("Kept again."), m.premise(now + 2))
check("a restated premise is stripped", m.strip_restated_premise("A black stick in the dark. It's just a line that hasn't started yet.", "A black stick in the dark.") == "It's just a line that hasn't started yet.")
...
m, M = fresh_mind()
m.absorb("A morning thought. It's the same loop.", "reflection", "12:50. Something settles.", now - 3 * 3600)
...
check("a surprise look is a surprise tempo", lk["tempo"] == "surprise", lk["tempo"])
m2, _ = fresh_mind()
m2.absorb("A thought.", "think", "c", now - 60)
m2.absorb("A settled paragraph.", "reflection", "c", now - 30)
check("after a reflection: a new thread's room", m2.build("think", now, Agent(), {}, None)["tempo"] == "new_thread")
