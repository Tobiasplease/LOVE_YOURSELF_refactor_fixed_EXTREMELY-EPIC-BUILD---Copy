        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    # Simulate dynamic consciousness data with startle spikes
                    base_time = current_time * 0.1
                    mood_wave = (math.sin(base_time * 0.3) + math.sin(base_time * 0.7)) * 0.5
                    
                    # Add periodic startle spikes for testing
                    startle_cycle = current_time * 0.05  # Very slow cycle for testing
                    startle_spike = 0
                    if abs(math.sin(startle_cycle)) > 0.98:  # Rare sharp spikes
                        startle_spike = 0.5  # Boost novelty during startle
                    
                    base_novelty = abs(math.sin(base_time * 0.5)) * 0.8
                    final_novelty = min(1.0, base_novelty + startle_spike)
                    
                    with self.data_lock:
                        self.consciousness_data.update({
                            'mood': mood_wave,
                            'novelty': final_novelty,  # Now includes startle spikes
